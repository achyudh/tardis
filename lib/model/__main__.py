...
if __name__ == '__main__':
    ...
    if args.dataset == 'en_de':
        encoder_train_input, decoder_train_input, decoder_train_target, source_vocab_size, target_vocab_size = \
            fetch.en_de(args.dataset_path)
        # encoder_test_input, decoder_test_input, decoder_test_target, source_vocab_size, target_vocab_size = \
        #     fetch.en_de(args.dataset_path, splits='test')
    elif args.dataset == 'de_en':
        encoder_train_input, decoder_train_input, decoder_train_target, source_vocab_size, target_vocab_size = \
            fetch.en_de(args.dataset_path, reverse=True)
        # encoder_test_input, decoder_test_input, decoder_test_target, source_vocab_size, target_vocab_size = \
        #     fetch.en_de(args.dataset_path, reverse=True, splits='test')
    else:
